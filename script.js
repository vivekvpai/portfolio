...
function eraseText() {
    typingInterval = setInterval(function () {
        if (greetingElement.textContent.length > 0) {
            greetingElement.textContent = greetingElement.textContent.slice(
                0,
                -1
            );
        } else {
            ...
            currentGreetingIndex =
                (currentGreetingIndex + 1) % greetings.length;
            updateGreeting();
        }
    }, 100);
}
...
// =================================================================================================
const startDate = new Date(2022, 6); // Month is zero-based
...
document.getElementById(
    "current-time"
).innerText = `${years} years ${months} months`;
